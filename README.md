...
Все активные задачи Todoist хранятся во `Входящих` и распределяются по четырём разделам из базы
`STREAMS`: `РАБОТА`, `БИЗНЕС`, `СЕМЬЯ`, `ЛИЧНОЕ`. Раздел определяется полем `TASKS.Stream`; если оно
пустое, используется Stream связанного проекта из `PROJECTS`.
